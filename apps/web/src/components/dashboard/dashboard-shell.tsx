...
export function DashboardShell({ campaigns, characters }: DashboardShellProps) {
  return (
    <SidebarProvider
      style={
        {
    ...
    >
      ...
      <SidebarInset className="tk-dashboard-canvas min-h-svh overflow-hidden">
        <header className="sticky top-0 z-20 flex h-14 shrink-0 items-center border-b bg-background/85 px-4 backdrop-blur-md sm:px-6">
          ...
          <span className="ml-3 font-mono text-[10px] text-muted-foreground uppercase tracking-[0.16em]">
            Dashboard
          </span>
        </header>

        <div className="relative mx-auto flex w-full max-w-[92rem] flex-1 flex-col px-4 py-8 sm:px-6 sm:py-10 lg:px-10">
              ...
              emptyState={
                <Empty className="border">
                  <EmptyHeader>
                    ...
                    <EmptyDescription>
                      Your character sheets will be ready here when you make
                      one.
                    </EmptyDescription>
                  </EmptyHeader>
                </Empty>
